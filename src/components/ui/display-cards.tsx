...
function DisplayCard({
  ...
  return (
    ...
    >
      <div>
        <span className={cn("relative inline-block rounded-full p-1", iconClassName?.includes("blue") ? "bg-blue-800" : iconClassName?.includes("yellow") ? "bg-yellow-600" : "bg-blue-800")}>
          {icon}
        </span>
        <p className={cn("text-lg font-medium", titleClassName)}>{title}</p>
      </div>
      <p className="whitespace-normal text-lg">{description}</p>
